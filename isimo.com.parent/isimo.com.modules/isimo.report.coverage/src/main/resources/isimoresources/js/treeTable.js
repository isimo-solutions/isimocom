var com_github_culmat_jsTreeTable =  (function(){
	...
	function renderTree(tree, childrenAttr, idAttr, attrs, renderer, tableAttributes, tdAttributes) {
		childrenAttr = childrenAttr || 'children'
		idAttr = idAttr || 'id'
		...
		function render(node, parent) {
			...
			if (renderer) {
				...
				$.each(attrs, function(attr, desc) {
					$(tr).append($('<td class="'+attr+'">' + node[attr] + '</td>'))
				})
			} else {
	...
	function appendTreetable(tree, options) {
		...
		var rendered = renderTree(tree, options.childrenAttr, options.idAttr,
				options.renderedAttr, options.renderer, options.tableAttributes, options.tdAttributes)
